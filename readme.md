...
│   │   ├── eq_dag_pandas.py   # Airflow DAG for the Pandas pipeline
│   │   └── eq_dag_pyspark.py  # Airflow DAG for the PySpark pipeline
...
│   │   ├── postgres_loader.py # PostgreSQL database loader for Pandas
│   │   └── schemas.py    # Database schemas and DDL helpers
│   └── pyspark/          # PySpark pipeline source code
...
│       ├── main.py       # PySpark entry point
│       └── flow.md       # PySpark local setup guide
├── .env                  # Project environment configuration file
...
* **Bronze Layer (Raw):** Ingests raw data from the Kaggle Earthquake Dataset, adds ingestion metadata (`ingest_timestamp`, `ingest_date`), and stores it in Parquet format partitioned by `ingest_date`.
* **Silver Layer (Cleansed & Enriched):**
    * Standardizes column names (lowercase with underscores) and data types.
    * **Deduplication & Validation:** Removes duplicate records based on the composite key (`magnitude`, `date_time`, `latitude`, `longitude`) and filters out invalid coordinates/magnitudes (e.g. magnitude <= 0).
...
* **Gold Layer (Aggregated):**
    ...
    * Creates country-level aggregate statistics (earthquake frequency, average magnitude, maximum/minimum magnitudes, and potential tsunami counts) saved in `gold.country_stats`.
...
* **Key Libraries:** `pyspark`, `pandas`, `reverse_geocoder`, `pycountry-convert`, `sqlalchemy`, `psycopg2-binary`, `pyarrow`

---

## Main Transformation Logic

### 1. Deduplication (Idempotency) & Target Keys
To prevent duplicate records and ensure idempotency on pipeline reruns, the pipeline deduplicates records based on a composite key: `magnitude`, `date_time`, `latitude`, and `longitude`. Both engines perform deduplication using this key subset and execute upserts in PostgreSQL (using `INSERT ON CONFLICT DO UPDATE` based on these composite primary keys).
* **Pandas:**
  ```python
...
* **Optimized Execution:** To maximize performance, both pipelines filter and execute reverse geocoding only on records that actually have missing location info, unioning the results back. The PySpark implementation uses a Spark User Defined Function (UDF) to process the coordinate lookup.
...
3. **Trigger & Monitor Pipeline**
   * Open your browser and navigate to the Airflow UI at `http://localhost:8080` (Default credentials: `admin` / `admin` from the `.env` configuration).
   * Turn on and trigger either the `earthquake_data_pipeline_pandas` DAG or the `earthquake_data_pipeline_pyspark` DAG.

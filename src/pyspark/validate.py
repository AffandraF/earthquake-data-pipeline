...
def compare_dataframes(df1, df2, df1_name, df2_name, join_keys):
    ...
    for col in set(schema1.keys()).union(schema2.keys()):
        ...
        if t1 != t2:
            print(f"WARNING: Schema mismatch for column '{col}': {df1_name} = {t1} | {df2_name} = {t2}")
            schema_diff = True
    ...
    null_counts1 = df1.select([F.sum(F.col(c).isNull().cast("int")).alias(c) for c in df1.columns]).collect()[0].asDict()
    null_counts2 = df2.select([F.sum(F.col(c).isNull().cast("int")).alias(c) for c in df2.columns]).collect()[0].asDict()
    ...
    for col in df1.columns:
        ...
        if nc1 != nc2:
            print(f"WARNING: Null count mismatch for '{col}': {df1_name} = {nc1} | {df2_name} = {nc2}")
            null_diff = True
    if not null_diff:
...
def run_validation():
    ...
    # Run Comparisons
    compare_dataframes(silver_pd, silver_sp, "silver_pandas", "silver_pyspark", ["magnitude", "date_time", "latitude", "longitude"])
    compare_dataframes(gold_pd, gold_sp, "gold_pandas", "gold_pyspark", ["magnitude", "date_time", "latitude", "longitude"])
